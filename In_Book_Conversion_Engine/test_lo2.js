...
const lo2 = result.find(l => l.id === 'LO2' && l.ce.filter(q => q.options.length > 0).length > 0);
if (lo2) {
    console.log(`LO: ${lo2.id}`);
    lo2.ce.filter(q => q.options.length > 0).forEach(q => {
        console.log(`Q${q.questionN}: ${q.question}`);
        q.options.forEach(o => {
            console.log(`  Ans: ${o.answer} (${o.diagnosticCode}) ${o.isCorrect ? '[CORRECT]' : ''}`);
        });
    });
} else {
    console.log('LO2 not found or empty.');
}
